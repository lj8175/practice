...
#define MAX_PROC_GROUP_NUM  100			//最大进程组个数
...
namespace tbase
{
    namespace tprocmon
    {
        ...
        typedef struct {
            ...
            unsigned short brate_;	//回包成功率
            unsigned short delay_;	//平均延时
            unsigned fd_num_;		//句柄数
        ...
        public:
            ...
            inline void set_start_mode(const spp::dll_mode mode);
            void set_ctrl_cmdline(const char**argv, const int argc);
            void set_msglog_id(int id);
            void parse_group_type();
        protected:
            CCommu* commu_;
