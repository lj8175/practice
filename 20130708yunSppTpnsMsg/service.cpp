...
#include "msg.h"
#include "GetState.h"
#include "tpns/tpns_header.h"

#define MODULE_PROC_NUM	"module_proc_num"

char *format_time( time_t tm);
CTpnsMsgManager g_msgManager;


int Init(CAsyncFrame* pFrame, CMsgBase* pMsg)
{
    //CAsyncFrame *pFrame = (CAsyncFrame *)pFrame;
    CMsg *msg = (CMsg *) pMsg;
    int state = msg->Process();
    pFrame->FRAME_LOG( LOG_DEBUG, "%s\n", msg->m_tpr.msgId.c_str());
    pFrame->FRAME_LOG( LOG_DEBUG, "%s\n", msg->m_tpr.token.c_str());
    return state;
}
...
int Fini(CAsyncFrame* pFrame, CMsgBase* pMsg)
{
    ...
    blob_type rspblob;
	rspblob.data = "Hello World";
	rspblob.len = sizeof("Hello World");
...
extern "C" int spp_handle_init(void* arg1, void* arg2)
{
    ...
    {
        g_msgManager.Register(CMD_PUSH, new CMsg);
...
extern "C" int spp_handle_input(unsigned flow, void* arg1, void* arg2)
{
    ...
    if (blob->len < (int32_t)(sizeof(TpnsHeader)+sizeof(TpnsContent))) 
    {
        return 0;
    }
    //TpnsHeader *th = (TpnsHeader *)(blob->data);
    TpnsContent *tc = (TpnsContent*)(blob->data+2);
    //服务器容器对象
    CServerBase* base = (CServerBase*)arg2;
    base->log_.LOG_P(LOG_DEBUG, "spp_handle_input, %d, %d, %s, %s, data length: %d \n", 
            flow, 
            blob->len, 
...
extern "C" int spp_handle_process(unsigned flow, void* arg1, void* arg2)
{
    ...
    CMsg *msg = (CMsg*)g_msgManager.GetMsg((uint8_t*)blob->data, blob->len);
    if (!msg)
    {
	    base->log_.LOG_P_PID(LOG_DEBUG, "spp_handle_process can not create CMsg .." );
        return -1;
    }
