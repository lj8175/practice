...
int main(int argc, char** argv)
{
    ...
    while (1){
       ...
       select(maxfd+1, &ready_set, NULL, NULL, &tval);
       ...
       if(FD_ISSET(clifd, &ready_set))
       {
            ...
            int s = 1 + (int) (100.0 * (rand() / (RAND_MAX + 1.0)));
            int pid = 0;
            ...
            if((pid=fork())==0)
            {
                ...
                timestamp = time(NULL);
                strcpy(buf2, "Timestamp in server: " );
                strcat(buf2,ctime(&timestamp));
                snprintf(buf2+strlen(buf2)-1, BUFFER_SIZE-strlen(buf2)+1, " [%s] send after sleep [%d] second\n", buf, s);
                sleep(s);
