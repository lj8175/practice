...
int main(){	
	...
	result=(char *)malloc(length+2);
        count=(int *)malloc((length+1)*sizeof(int));
	memset(count,0,((length+1)*sizeof(int)));
	int i;
	*(result+length+1)=0;
...
void phonekey(int level){
  ...
  if(level==length)	{
    while((result[level]=c[num][count[level]++])!=0) {printf("%s\n",result);total++;}
    count[level]=0; return;	
  }
  while((result[level]=c[num][count[level]])!=0){ phonekey(level+1);count[level]++;}
  for(i=level;i<=length;i++) count[i]=0;
}
